...
def make_geojson():
    ...
    for row in data:
        FeatureCollection["features"].append(
            {"type": "Feature", "geometry": {"type": "Point", "coordinates": [row["Long_"], row['Lat']]},
             "properties": {"COUNTRY": row['Country_Region'], 'CONFIRMED': int(row['Confirmed']),
                            'DEATHS': int(row['Deaths']), 'RECOVERED': int(row['Recovered'])}})
