...
def get_ethnicity_insights_df():
    ...
    filtered_df = form_data[["ethnicity", "probable"]]
    ethnicity_probable = filtered_df.groupby("ethnicity", as_index=False).mean().sort_values(by="probable", ascending=False)
    ethnicity_probable["count"] = filtered_df.groupby("ethnicity", as_index=False).count()["probable"]

    return ethnicity_probable
...
def upload_blob(ethnicity_probable):
    ...
    blob = bucket.blob(UPLOAD_ETHNICITY)
    ...
    print(
        "File {} uploaded to {}.".format(
            UPLOAD_ETHNICITY, GCS_BUCKET
        )
    )

def main():
    print("Getting ethnicity insights...")
    ethnicity_probable = get_ethnicity_insights_df()

    upload_blob(ethnicity_probable)
